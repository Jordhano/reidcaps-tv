 ...
 (function($) {
	...
	$.fn.jTPS = function ( opt ) {
		...
		$(this).data('tableSettings', $.extend({
			...
			perPageText:		'por pagina:',						// text that appears before perPages links
			...
			function () {
				...
				$(this).bind('click', 
					function () { 
						...
						$('>.nav .status', pT).html( 'Viendo ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' de ' + rowCount + '' );
						clearSelection();
						// callback function after pagination renderd
						$(pT).data('tableSettings').clickCallback();
					}
		...
		$('>.nav .status', this).html( 'Viendo ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' de ' + rowCount );
